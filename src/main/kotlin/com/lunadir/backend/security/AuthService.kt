...
class AuthService(
    ...
    /**
     * Create a new User with the provided email and the hashed password.
     *
    ...
    /**
     * Check the provided user credentials and return a newly generated token pair.
     *
    ...
    /**
     * Generate a new token pair (access and refresh tokens) if a valid refresh token
    ...
    fun refresh(refreshToken: String): JwtService.TokenPair {
        ...
        val tokenIsValid: Boolean = jwtService.validateRefreshToken(refreshToken)
        if (!tokenIsValid) {
        ...
        // Check if the refresh token is not already used, i.e. its hash is still in the database
        val hashedRefreshToken = jwtService.hashRefreshToken(refreshToken)
        refreshTokenRepository.findByUserIdAndHashedToken(userId, hashedRefreshToken)
        ...
        refreshTokenRepository.deleteByUserIdAndHashedToken(userId, hashedRefreshToken)
        return jwtService.generateTokenPair(user)
    }
}
